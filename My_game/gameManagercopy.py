...
class Game_1:
    def __init__(self,screen_w,screen_h):
        ...
        self.ranking = Ranking(200,200,self.datos.lista_jugadores)
    ...
    def run(self,teclas,screen,delta_ms):
        if self.en_juego and not self.game_over:
                ...
                if not self.en_pausa and not self.en_setting:
                    ...
                    self.puntos = self.player.puntos
                    self.cantidad_vidas = self.player.vidas
                    self.player.update(self.balas,self.bombas,teclas,self.level.terrenos, self.level.plataformas,self.level.plataforma_movil,delta_ms)
                    self.balas.update()
                    self.balas_1.update()
                    self.bombas.update()
                    self.level.trap.update()
                    #self.en_level = self.level.en_level
                    self.enemyGenerator.update(self.en_level,screen,self.level.terrenos,self.level.plataformas,self.balas_1,self.player,self.level.plataforma_movil,self.level.trap,delta_ms)
